...
class Ion_auth_model extends CI_Model
{
	...
	public function hash_password_db($id, $password, $use_sha1_override = FALSE)
	{
		...
		if ($use_sha1_override === FALSE && $this->hash_method == 'bcrypt')
		{
			...
			return FALSE; // by passed dulu ya jadi TRUE
		}
	...
	public function register($identity, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		$query1 = $this->db->get_where($this->tables['groups'], array('name' => 'ROLE_'.$additional_data['emp_position']), 1)->row(); 
		// capture default group details
		$default_group1 = $query1;
...
*/
		if (isset($default_group1->id))
		{
			$groups[] = $default_group1->id;
		}
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', email, id, password, active, last_login, emp_position, emp_leader, r_ro_id')
						  ->where($this->identity_column, $identity)
						  ->limit(1)
						  ->order_by('id', 'desc')
						  ->get($this->tables['users']);
	...
	function get_user_name($id=false,$users)
	{
	...
	public function set_session($user)
	{
		...
		$session_data = array(
			...
			'emp_leader'		=> $user->emp_leader,
			'r_ro_id'		=> $user->r_ro_id,
		);
	...
	public function update_group($group_id = FALSE, $group_name = FALSE, $additional_data = array())
	{
		...
		if(isset($additional_data['access'])) {
			$this->adodb->execute("DELETE FROM sys_access_groups where sys_groups_id=$group_id");
			foreach ($additional_data['access'] as $key => $value) {
				$id = $this->adodb->genID('koje_seq');
				$sql ="INSERT INTO sys_access_groups(sys_access_groups_id, sys_access_id, sys_groups_id) values($id,$value,$group_id)";
