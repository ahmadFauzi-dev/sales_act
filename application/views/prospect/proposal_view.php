		...
		function index($params=false) {
			...
			if ($loginID == 1) {
				$query = array(	 "from" 	=> "FROM v_prospect_current a",
							 "where"  => "WHERE status='$status' and t_stage_id=$t_stage_id and presales_status = 'PRESALES-02'",
							 "params" => array()
							 );
			} else {
				$query = array(	 "from" 	=> "FROM v_prospect_current a",
							 "where"  => "WHERE sales_id=$loginID and status='$status' and t_stage_id=$t_stage_id and presales_status = 'PRESALES-02'",
							 "params" => array()
							 );
			}
			//
