...
*/
	$myView = new escalation_quotation_view();
	if (!isset($VIEW)) $VIEW = $this->router->method;
	$myView->$VIEW($VARS);

	class escalation_quotation_view {
		var $CI;
		...
		function index($params=false) {
			...
			$status = $this->CI->app_lib->getQuotationStatus('ESCALATION');

			$str = $table->renderDataTable(
				array(
					'id' 		=> $pagerID,
					'search'=> '&nbsp;',
					'title' => "Request Escalation Quotation",
					'primary_key' => 't_quotation_id',
					"query" => array(	 "from" 	=> "FROM v_quotation a",
														 "where"  => "WHERE status='$status' and utils_is_leader_from($loginID,request_by)='Y'",
														 "params" => array(),
										),
					'order' => array(
														PAGER::COL2=>'asc',
