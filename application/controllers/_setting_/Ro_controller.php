...
class ro_controller extends KOJE_Controller {
	...
	function __construct() {
		...
		$this->params = array();
		$this->load->model('_setting_/ro_model');
		$this->view  = '_setting_/ro_view';
		$this->params['INPUT'] = $this->CI->input->post();
	}
	function index()
	{
		$this->params['FORMS'] 		= $this->ro_model->getForm('form_search',$this->params);
    ...
    $this->params['DATA']				= array();
		...
		$this->params['FORMS'] = $this->ro_model->getForm($this->params['MODEL'],$this->params);
		...
		{
			$this->params['RESULT'] 	= $this->ro_model->do_insert($this->params);
	...
	function edit($params=false)
	{
		...
		$this->params['VIEW']       = "view_general";
		$this->params['DATA']     	= $this->CI->adodb->GetRow("select * from r_ro where r_ro_id=?", array($this->input->get_post('r_ro_id')));
		$this->params['FORMS'] = $this->ro_model->getForm($this->params['MODEL'],$this->params);
		$this->koje_system->array_set_element($this->params['FORMS']['item'], $this->params['DATA'], 'value');
		$this->koje_validation->set_rules($this->params['FORMS']['item']);
		if ($this->koje_validation->run() == FALSE)
		...
		{
			$this->params['RESULT'] 	= $this->ro_model->do_edit($this->params);
			$this->params['SUBMIT'] 	= true;
		}
	...
	function delete($params=false)
	{
		...
		{
			$this->params['RESULT'] 	= $this->ro_model->do_delete($this->params);
			$this->params['SUBMIT'] 	= true;
		}
